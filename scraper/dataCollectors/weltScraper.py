...
if __name__ == '__main__':
    ...
    # else:
    #     run_regular()
    weltScraper = WeltScraper()
    todo = weltScraper.get_article_list(date(2020, 1, 20), date(2020, 1, 21))
    cmts = []
    for t in todo:
        t.set_header_id(1)
        t.set_body_id(1)
        weltScraper.get_article_details(t)
        cmts += weltScraper.get_comments_for_article(t)
    for t in todo[0:10]:
        t.print()
    for c in cmts[0:10]:
        c.print()
